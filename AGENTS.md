...
- **No tool sets a schematic symbol's "Exclude from BOM" / "Exclude from Board" flags** (`in_bom`/`on_board`) — `edit_schematic_component` doesn't expose either. A documentation-only symbol (module sourced/assembled by hand, kept in the schematic for reference) has to be marked up by hand-editing the `.kicad_sch` file directly; track upstream issue #435.

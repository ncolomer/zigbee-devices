...
**ZCL Configure Reporting sentinel values (never use these as literal intervals):**
- `max: 0xFFFF` does not mean "report at most every 65535s" — per ZCL spec §2.5.7.1.6 it means
  **terminate/cancel reporting** for that attribute. `zigbee-herdsman` implements this literally
  (`Endpoint.configureReporting()` skips recording the attribute as configured whenever
  `maxRepIntval === 0xffff`), so a `reporting: {max: 0xffff, ...}` in a converter silently
  disables reporting — no error anywhere, device still ACKs the command, Reporting tab shows
  `None`. Use a real interval (e.g. `65000`) for an "infrequent heartbeat" instead.
- `max: 0x0000` + `min: 0xFFFF` together mean **revert to default reporting config** — another
  non-literal sentinel pair to avoid unless that's actually the intent.
